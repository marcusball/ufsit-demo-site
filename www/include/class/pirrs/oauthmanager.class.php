...
class OAuthManager{
     ...
     * returns the new /pirrs/user/AccessToken object, or false if unable.
     */
            ...
            $refreshRequest->finally(function($response) use(&$accessToken){
                    ...
                    if($response['status_code'] == 200){
                        if(isset($response['access_token'])){
                            ...
                            if($update !== false){
                                $newToken = new \pirrs\user\AccessToken();
     ...
     */
    public function setAccessToken(\pirrs\user\AccessToken $accessToken){
    ...
    private function GET($url, Array $params = null){
        ...
        if($oma !== false){
            ...
            return $oma->GET($this->generateUrl($url),$params,array('User-Agent' => 'pirrs-web'));
        }
        return false;
    }
    ...
    private function POST($url, Array $params = null){
        ...
        if($oma !== false){
            ...
            return $oma->POST($this->generateUrl($url),$params,array('User-Agent' => 'pirrs-web'));
        }
        return false;
    }
